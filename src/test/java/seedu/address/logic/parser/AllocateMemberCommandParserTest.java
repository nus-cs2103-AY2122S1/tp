...
public class AllocateMemberCommandParserTest {
    private AllocateMemberCommandParser parser = new AllocateMemberCommandParser();

    @Test
    public void parse_emptyArgs_exceptionThrown() {
        assertParseFailure(parser, "  ",
                String.format(MESSAGE_INVALID_COMMAND_FORMAT, AllocateMemberCommand.MESSAGE_USAGE));
    }

    @Test
    public void parse_validArgs_success() {
        assertParseSuccess(parser, " 1   1 1",
                new AllocateMemberCommand(INDEX_FIRST, INDEX_FIRST, DayOfWeek.MONDAY));
