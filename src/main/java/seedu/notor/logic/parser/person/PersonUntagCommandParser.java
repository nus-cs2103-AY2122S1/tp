...
/**
 * Checks the parsing for the untag command, and returns a Person Command when parse is called.
 */
public class PersonUntagCommandParser extends PersonCommandParser {
    public static final String COMMAND_WORD = PersonUntagCommand.COMMAND_WORD;

    public PersonUntagCommandParser(String unparsedIndex, String arguments) throws ParseException {
     ...
     */
    public PersonUntagCommand parse(String args) throws ParseException {
        requireNonNull(args);
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_TAG);

        if (!argMultimap.getValue(PREFIX_TAG).isPresent()) {
            throw new ParseException(PersonUntagCommand.MESSAGE_NO_TAGS);
        }
        ...
        return new PersonUntagCommand(index, ParserUtil.parseTags(tags));
    }
