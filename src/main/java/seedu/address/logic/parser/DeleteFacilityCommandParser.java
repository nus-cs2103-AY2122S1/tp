...
public class DeleteFacilityCommandParser implements Parser<DeleteFacilityCommand> {
     ...
     * and returns a DeleteFacilityCommand object for execution.
     *
     * @throws ParseException if the user input does not conform the expected format.
     */
    public DeleteFacilityCommand parse(String args) throws ParseException {
        try {
            Index index = ParserUtil.parseIndex(args);
            return new DeleteFacilityCommand(index);
        } catch (ParseException pe) {
            throw new ParseException(
                    String.format(MESSAGE_INVALID_COMMAND_FORMAT, DeleteFacilityCommand.MESSAGE_USAGE), pe);
        }
    }
}
