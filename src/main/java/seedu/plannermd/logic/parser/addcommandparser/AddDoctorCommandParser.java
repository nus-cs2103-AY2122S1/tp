...
public class AddDoctorCommandParser implements Parser<AddDoctorCommand> {
    ...
    public AddDoctorCommand parse(String args) throws ParseException {
        ...
        // Check for risk parameter ine case users are confused and use the risk field with doctors
        if (argMultimap.getValue(PREFIX_RISK).isPresent()) {
            throw new ParseException(String.format(MESSAGE_INVALID_DOCTOR_RISK_FIELD, AddDoctorCommand.MESSAGE_USAGE));
        }
