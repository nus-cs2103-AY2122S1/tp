...
* **Update Feature**: Updated the Help Window, with a better UI - a darker theme. Added the list of commands supported by the app for quick access. Also added a `visit URL` button, which when clicked open, opens the User Guide in the Web Browser.
