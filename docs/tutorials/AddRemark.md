...
Now that we have our `RemarkCommand` ready to be executed, we need to update `TutorAidParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
:information_source: Don’t forget to update `TutorAidParser` to use our new `RemarkCommandParser`!
...
:exclamation: You must delete AddressBook’s storage file located at `/data/tutoraid.json` before running it! Not doing so will cause AddressBook to default to an empty address book!
