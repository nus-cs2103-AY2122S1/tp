...
/**
 * Changes the remark of an existing staff in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the staff identified "
            + "by the index number used in the last staff listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the staff in the filtered staff list to edit the remark
     * @param remark of the staff to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of staff data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the staff’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a staff.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.staff`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each staff.
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedStaff = new Person(
                staffToEdit.getName(), staffToEdit.getPhone(), staffToEdit.getEmail(),
                staffToEdit.getAddress(), remark, staffToEdit.getTags());

        model.setPerson(staffToEdit, editedStaff);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedStaff));
    }
     ...
     * {@code staffToEdit}.
     */
    private String generateSuccessMessage(Person staffToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, staffToEdit);
    }
```
