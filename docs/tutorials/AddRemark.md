...
Now that we have our `RemarkCommand` ready to be executed, we need to update `RhrhParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
:information_source: Don’t forget to update `RhrhParser` to use our new `RemarkCommandParser`!
...
Rhrh stores data by serializing `JsonAdaptedPerson` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedPerson` to work with our new `Person`!
...
This concludes the tutorial for adding a new `Command` to Rhrh.
