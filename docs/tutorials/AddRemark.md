...
/**
 * Changes the remark of an existing anime in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the anime identified "
            + "by the index number used in the last anime listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the anime in the filtered anime list to edit the remark
     * @param remark of the anime to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.anilist.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of anime data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the anime’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a anime.

### Add a new `Remark` class

Create a new `Remark` in `seedu.anilist.model.anime`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each anime.

Simply add the following to [`seedu.anilist.ui.AnimeCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
``` java
public PersonCard(Person anime, int displayedIndex) {
    //...
    remark.setText(anime.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person animeToEdit = lastShownList.get(index.getZeroBased());
        Person editedAnime = new Person(
                animeToEdit.getName(), animeToEdit.getPhone(), animeToEdit.getEmail(),
                animeToEdit.getAddress(), remark, animeToEdit.getTags());

        model.setPerson(animeToEdit, editedAnime);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedAnime));
    }
     ...
     * {@code animeToEdit}.
     */
    private String generateSuccessMessage(Person animeToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, animeToEdit);
    }
```
